get.otuloc <- function (bcnt, optlist = NULL, ndc = TRUE, outputFile = FALSE,
    gui = FALSE)
{
    names0 <- names(bcnt)
    siteid <- names0[1]
                ...
                for (i in 1:length(spec)) {
                  if (is.na(match(spec[i], optlist.spec))) {
                    ...
                    repeat {
                      ...
                      spec.half <- substring(spec.half, w2 +
                        attributes(w2)$match.length, nchar(spec.half))
                      k <- k + 1
                    }
...
#        otufin <- levels(df2$TNAME)[df2$TNAME]
    ...
    if (outputFile) {
        write.table(df2, file = "sum.otu.txt", sep = "\t",
            row.names = FALSE)
